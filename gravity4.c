...
typedef struct projectile{
    ...
    p_color_t color;
    p_state_t state;
    p_split_t splittable;
    double y_total; //highest point in path
    ...
    while (keep_going){
        ...
        for (j = 0; j < np; j++){
            ...
            if (p[j].state == ACTIVE){
                ...
                if (p[j].splittable == SPLIT1){
                    if (p[j].y_total >= 2 * p[j].y){
                        p[j].state = INACTIVE;
                        for (k = 0; k < 4; k++){
                            p[np].state = ACTIVE;
                            p[np].x0 = p[np].x = p[j].x;
...
void prepare_pic (unsigned char *picdat){
    ...
    for (i = 0; i < rows; i++){
        for (j = 0; j < cols; j++){
            picdat [ri (i, j, cols)] = floor (picdat [ri (i, j, cols)] * .9 + .5);
            picdat [gi (i, j, cols)] = floor (picdat [gi (i, j, cols)] * .9 + .5);
            picdat [bi (i, j, cols)] = floor (picdat [bi (i, j, cols)] * .9 + .5);
