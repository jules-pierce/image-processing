...
void prepare_pic (unsigned char *picdat, accel_t *a);
...
void prepare_pic (unsigned char *picdat, accel_t *a){
    ...
    if ((a->x2 - a->x1) == 0) slope = 1000;
    else slope = (a->y2 - a->y1)/(a->x2 - a->x1);
    
    for (i = a->y1; i < a->y2; i++){
        for (j = a->x1; j < a->x2; j++){
            //printf ("%d\n", j);
            if (i == (slope * (j - a->x1)) + a->y1){
...
void check_accels(accel_t *a, puck_t *p, unsigned char *picdat){
    ...
    if (x > ax1 && x < ax2 && y > ay1 && y < ay2){
        ...
        if ( (p->x0 <= x && p->x >= x) &&
            ...
            p->vx = (sqrt ((p->vx * p->vx) + (p->vy * p->vy)) + a->v ) * (cos (atan (slope2)));
            p->vy = (sqrt ((p->vx * p->vx) + (p->vy * p->vy)) + a->v ) * (sin (atan (slope2)));
